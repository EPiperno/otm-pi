...
class CameraThread:
    def __init__(self, device=0, width=None, height=None, fps=None, jpeg_quality=80, downscale=1, frame_skip=0, roi=None):
        self.device = device
        self.width = width
        self.height = height
        self.fps = fps
...
class DahengCameraThread:
    ...
    def __init__(self, index=1, width=None, height=None, fps=None, serial=None, jpeg_quality=80, downscale=1, frame_skip=0, roi=None):
...
def create_app() -> Flask:
    ...
    if backend.startswith('daheng'):
        ...
        if serial_raw:
            ...
            if sr:
                serial = sr
        cam = DahengCameraThread(index=dh_index, serial=serial, width=w_i, height=h_i, fps=fps, jpeg_quality=jpeg_quality, downscale=downscale, frame_skip=frame_skip, roi=roi)
    else:
        ...
        if isinstance(device, str) and device.startswith('/dev/video'):
            try:
                ...
                pass
        cam = CameraThread(device=device_index, width=w_i, height=h_i, fps=fps, jpeg_quality=jpeg_quality, downscale=downscale, frame_skip=frame_skip, roi=roi)
    ...
    def video_settings():
        if request.method == 'POST':
            ...
            try:
                ...
                # Perform full reconfigure if any change requested
        ...
        return jsonify({'resolution': f"{cam.width}x{cam.height}" if cam.width and cam.height else None, 'fps': cam.fps})
